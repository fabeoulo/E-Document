...
//            guiStyle: "bootstrap",
            ...
            colModel: [
                ...
                {label: 'PRE-ASSY', name: "preAssy.id", edittype: "select", editoptions: {value: selectOptions["preAssy"], dataEvents: preAssy_select_event_m4f, defaultValue: preAssy_default_value_m4f.toString()}, formatter: selectOptions["preAssy_func"], width: 100, searchrules: {required: true}, stype: "select", searchoptions: {value: selectOptions["preAssy"], sopt: ['eq']}},
                {label: 'BAB_FLOW', name: "flowByBabFlowId.id", edittype: "select", editoptions: {value: selectOptions["bab_flow"], dataEvents: babFlow_select_event_m4f, defaultValue: babFlow_default_value_m4f.toString()}, formatter: selectOptions["bab_flow_func"], cellattr: hideEmptyBabFlow, width: 100, searchrules: {required: true}, stype: "select", searchoptions: {value: selectOptions["bab_flow"], sopt: ['eq']}},
                {label: 'TEST_FLOW', name: "flowByTestFlowId.id", edittype: "select", editoptions: {value: selectOptions["test_flow"], dataEvents: testFlow_select_event_m4f, defaultValue: testFlow_default_value_m4f.toString()}, formatter: selectOptions["test_flow_func"], width: 100, searchrules: {required: true}, stype: "select", searchoptions: {value: selectOptions["test_flow"], sopt: ['eq']}},
                {label: 'PACKING_FLOW', name: "flowByPackingFlowId.id", edittype: "select", editoptions: {value: selectOptions["pkg_flow"], dataEvents: pkgFlow_select_event_m4f, defaultValue: pkgFlow_default_value_m4f.toString()}, formatter: selectOptions["pkg_flow_func"], width: 140, searchrules: {required: true}, stype: "select", searchoptions: {value: selectOptions["pkg_flow"], sopt: ['eq']}, formoptions: {elmsuffix: "<b class='danger'>確認秤重途程</b>"}},
                ...
                {label: 'Wifi', name: "wifi", width: 100, searchrules: number_search_rule, searchoptions: search_decimal_options, editrules: {number: true}, editoptions: {defaultValue: '0'}},
                ...
                {label: 'Test Profile', name: "testProfile", width: 100, hidden: true, editable: true, search: true, searchrules: {required: true}, searchoptions: search_string_options, edittype: "select", editoptions: {value: "0:0;M5:M5;601:601;B-B:B-B;EKI9528:EKI9528;EKI9516:EKI9516;EKI9520:EKI9520;ITA:ITA;EKI-9508:EKI-9508", defaultValue: "0", dataEvents: testProfile_select_event_m4f}},
                ...
                {edit: isUpdatable || isFullTableControllable, add: isFullTableControllable, del: isFullTableControllable, search: true},
                {
                    ...
                    beforeShowForm: function (form) {
                        setTimeout(function () {
                            ...
                            $("#preAssy\\.id, #flowByBabFlowId\\.id, #flowByPackingFlowId\\.id, #businessGroup\\.id").trigger("change");
                            checkLabelisEmpty();
                ...
                },
                {
                    ...
                    beforeShowForm: function (form) {
                        setTimeout(function () {
                            ...
                            $("#preAssy\\.id, #flowByBabFlowId\\.id, #flowByPackingFlowId\\.id, #businessGroup\\.id, #testProfile").trigger("change");
                            addNarPage();
                            addType();
                        }, 50);
                        greyout(form);
